...
const DashboardLayout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const navItems = [
    { name: 'Runtime Dashboard', path: '/', icon: LayoutDashboard },
    { name: 'Cluster Fleet', path: '/clusters', icon: Cloud },
    { name: 'Namespace Isolation', path: '/namespaces', icon: Layers },
    { name: 'Workload Catalog', path: '/workloads', icon: Container },
    { name: 'Blue/Green Deploy', path: '/deployments', icon: Terminal },
    { name: 'Runtime Security', path: '/security', icon: ShieldCheck },
    { name: 'Traffic Mesh', path: '/mesh', icon: Globe },
    { name: 'Resource Quotas', path: '/quotas', icon: Cpu },
    { name: 'Observability', path: '/monitoring', icon: BarChart3 },
    ...
    <div className="flex h-screen bg-slate-950 text-slate-200 overflow-hidden font-sans">
      ...
      <aside className={`bg-slate-900 border-r border-slate-800 transition-all duration-300 ${sidebarOpen ? 'w-64' : 'w-20'} flex flex-col`}>
        <div className="p-6 flex items-center gap-4 border-b border-slate-800">
          ...
          {sidebarOpen && <span className="font-bold text-xl tracking-tight bg-gradient-to-r from-white to-cyan-400 bg-clip-text text-transparent">RUNLAND</span>}
        </div>
          ...
          <div className="flex items-center gap-3 p-2 bg-slate-800/50 rounded-xl">
            ...
            {sidebarOpen && (
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold truncate">Platform Engineer</p>
                <p className="text-xs text-slate-500 truncate">Runtime Ops</p>
              </div>
            )}
          </div>
        </div>
        ...
        <header className="h-16 bg-slate-900/50 backdrop-blur-md border-b border-slate-800 flex items-center justify-between px-6 z-10">
          <div className="flex items-center gap-4 flex-1">
            ...
            <div className="relative max-w-md w-full">
              ...
              <input 
                type="text" 
                placeholder="Search clusters, workloads, or deployment logs..." 
